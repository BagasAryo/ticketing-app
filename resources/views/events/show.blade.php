            ...
            <div class="mt-4 space-y-4">
              @forelse($event->tickets as $ticket)
                <div class="card card-side shadow-sm p-4 items-center">
                  <div class="flex-1">
                    <h4 class="font-bold">{{ $ticket->tipe }}</h4>
                    <p class="text-sm text-gray-500">Stok: <span
                        id="stock-{{ $ticket->id }}">{{ $ticket->stok }}</span></p>
                    <p class="text-sm mt-2">{{ $ticket->keterangan ?? '' }}</p>
                  </div>

                  <div class="w-44 text-right">
                    <div class="text-lg font-bold">
                      {{ $ticket->harga ? 'Rp ' . number_format($ticket->harga, 0, ',', '.') : 'Gratis' }}
                    </div>

                    <div class="mt-3 flex items-center justify-end gap-2">
                      <button type="button" class="btn btn-sm btn-outline" data-action="dec"
                        data-id="{{ $ticket->id }}" aria-label="Kurangi satu">−</button>
                      <input id="qty-{{ $ticket->id }}" type="number" min="0" max="{{ $ticket->stok }}"
                        value="0" class="input input-bordered w-16 text-center" data-id="{{ $ticket->id }}" />
                      <button type="button" class="btn btn-sm btn-outline" data-action="inc"
                        data-id="{{ $ticket->id }}" aria-label="Tambah satu">+</button>
                    </div>

                    <div class="text-sm text-gray-500 mt-2">Subtotal: <span id="subtotal-{{ $ticket->id }}">Rp
                        0</span>
                    </div>
                  </div>
                </div>
